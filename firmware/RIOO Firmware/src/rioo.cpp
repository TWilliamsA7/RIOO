...
void runRIOO() {
    ...
    if (currentTime - lastLoopTime >= LOOP_INTERVAL) {
        ...
        // updateDebugScreen(gazeCommand.xPos, gazeCommand.zPos, 0, collisionSensorDistances[0], collisionSensorDistances[1]);

        // Only move ARM when the claw is not moving
        if (clawActionDone) {
            // Compute new target based on UART data
            Point target = computeTargetPoint(gazeCommand.xPos, gazeCommand.zPos);
...
Point computeTargetPoint(float rawX, float rawZ) {
    ...
    float targetX = (PLANAR_ALPHA * rawX) + ((1.0 - PLANAR_ALPHA) * prevTargetX);
    float targetZ = (PLANAR_ALPHA * rawZ) + ((1.0 - PLANAR_ALPHA) * prevTargetZ);
